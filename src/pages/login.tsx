  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success('Akun berhasil dibuat! 🎉 Silakan cek email untuk verifikasi, atau langsung login.');
        setMode('login');
        setPassword('');
        setConfirmPassword('');
      }
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4 animate-fade-in">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full btn-gold py-3 text-sm mt-2 flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
          ...
          {mode === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4 animate-fade-in">
              ...
              <button
                ...
                className="w-full btn-gold py-3 text-sm mt-2 flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {loading ? (
                  <>
                    <div className="w-4 h-4 border-2 border-charcoal-dark border-t-transparent rounded-full animate-spin" />
